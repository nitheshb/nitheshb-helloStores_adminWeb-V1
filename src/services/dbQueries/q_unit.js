...
export const  createBrandDb = async (orgId, payload)  =>  {
    ...
    try {
      const filesCollectionRef = collection(db, `T_unit`); // step 1: change to your collection name
...
let y  = {data:files, 
  ...
  "links": [
      ...
      {
          "url": "https:\/\/single-api.foodyman.org\/api\/v1\/dashboard\/admin\/T_unit\/paginate?page=1",
          "label": "1",
          "active": true
      },
      {
  ...
  "path": "https:\/\/single-api.foodyman.org\/api\/v1\/dashboard\/admin\/T_unit\/paginate",
  "per_page": "1000",
  "to": files.length,
  "total": files.length
}}
...
export const getAllBrandsSnap = async (params, callback) => {
  console.log('snap are ====>', params)
    ...
    const filesQuery1 = query(
      collection(db, `T_unit`), //step 1:  change to your collection name
      where("status", "==", params?.params?.status || "published")
    );
    const itemsQuery1 = query(
      collection(db, 'T_unit'),
    ...
    return unsubscribe;
      ...
      console.error('Error fetching T_unit:', error);
  }
  return     
  // const {params} = params
...
export const getAllBrandsById = async (orgId, uid, payload) => {
  try {
    const docRef = doc(db, `T_unit`, uid) // step 1: change to your collection name
...
x['images[0]'] = ""
    ...
    await updateDoc(doc(db, `T_unit`, uid), { // step 1: change to your collection name
      title: x.title,
      active: x.active
    })
    ...
    console.log('Failed updated T_unit', error, {
      ...params,
    })
...
export const deleteBrand= async (params) => {
  ...
  params.map(async(item) => {
   await deleteDoc(doc(db, 'T_unit', item)) // step 1: change to your collection name
  })
