...
export const  createBrandDb = async (orgId, payload)  =>  {
    ...
    try {
      const filesCollectionRef = collection(db, `brands`); // step 1: change to your collection name
...
export const getAllBrands = async (orgId, params) => {
  ...
  const filesQuery = query(
    collection(db, `brands`), // change to your collection name
...
export const getAllBrandsById = async (orgId, uid, payload) => {
  try {
    const docRef = doc(db, `brands`, uid) // step 1: change to your collection name
...
x['images[0]'] = ""
    ...
    await updateDoc(doc(db, `brands`, uid), { // step 1: change to your collection name
      title: x.title,
      active: x.active
    })
...
export const deleteBrand= async (params) => {
  ...
  params.map(async(item) => {
   await deleteDoc(doc(db, 'brands', item)) // step 1: change to your collection name
  })
