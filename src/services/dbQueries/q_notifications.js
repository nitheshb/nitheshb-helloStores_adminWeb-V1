...
export const  createBlogsNotificationsDb = async (orgId, payload)  =>  {
    ...
    try {
      const filesCollectionRef = collection(db, `T_notifications`); // step 1: change to your collection name
...
export const getAllBlogsNotifications = async (orgId, params) => {
  ...
  const filesQuery = query(
    collection(db, `T_notifications`), // change to your collection name
...
export const deleteBrand= async (params) => {
  ...
  params.map(async(item) => {
   await deleteDoc(doc(db, 'T_notifications', item)) // step 1: change to your collection name
  })
