...
export const  createBrandDb = async (orgId, payload)  =>  {
    ...
    try {
      const filesCollectionRef = collection(db, `T_discount`); // step 1: change to your collection name
...
export const getAllBrands = async (orgId, params) => {
  ...
  const filesQuery = query(
    collection(db, `T_discount`), // change to your collection name
...
export const getAllBrandsSnap = async (params, callback) => {
  console.log('snap are ====>', params)
    ...
    const filesQuery1 = query(
      collection(db, `T_discount`), //step 1:  change to your collection name
...
export const getAllBrandsById = async (orgId, uid, payload) => {
  try {
    const docRef = doc(db, `T_discount`, uid) // step 1: change to your collection name
...
x['images[0]'] = ""
    ...
    await updateDoc(doc(db, `T_discount`, uid), { // step 1: change to your collection name
      title: x.title,
      active: x.active
    })
    ...
    console.log('Failed updated T_discount', error, {
      ...params,
    })
...
export const deleteBrand= async (params) => {
  ...
  params.map(async(item) => {
   await deleteDoc(doc(db, 'T_discount', item)) // step 1: change to your collection name
  })
