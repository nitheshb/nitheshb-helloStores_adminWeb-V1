...
export const  createBrandDb = async (orgId, payload)  =>  {
    ...
    try {
      const filesCollectionRef = collection(db, `T_extra_values`); // step 1: change to your collection name
...
export const getAllBrands = async (orgId, params) => {
  ...
  const filesQuery = query(
    collection(db, `T_extra_values`), // change to your collection name
...
export const getAllBrandsSnap = async (params, callback) => {
  console.log('snap are ====>', params)
    ...
    const filesQuery1 = query(
      collection(db, `T_extra_values`), //step 1:  change to your collection name
...
x['images[0]'] = ""
    ...
    await updateDoc(doc(db, `T_extra_values`, uid), { // step 1: change to your collection name
      title: x.title,
      active: x.active
    })
...
export const deleteBrand= async (params) => {
  ...
  params.map(async(item) => {
   await deleteDoc(doc(db, 'T_extra_values', item)) // step 1: change to your collection name
  })
