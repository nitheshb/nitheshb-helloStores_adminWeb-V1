...
const ShopBonusEdit = () => {
  ...
  const { id } = useParams();
  const [hasMore, setHasMore] = useState({ bonusProductStock: false });
  ...
  useEffect(() => {
    if (activeMenu.refetch) getBonus(id);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeMenu.refetch]);
  ...
  async function fetchProductsStock({ page = 1, search = '' }) {
    const params = {
      page,
    ...
    return await productService.getStock(params).then((res) => {
      setHasMore((prev) => ({
        ...prev,
        bonusProductStock: !!res?.links?.next,
      }));
            ...
            <Col span={12}>
              ...
              >
                <InfiniteSelect
                  ...
                  hasMore={hasMore?.bonusProductStock}
                />
              </Form.Item>
            </Col>
            <Col span={12}>
